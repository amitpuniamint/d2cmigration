...
//Old Settings before march 2021
// function firdatecheck(){

// var firdatetime = document.getElementById("fir_date").value;
// var datepattern = /^(((0[1-9]|[12]\d|3[01])[\/\.-](0[13578]|1[02])[\/\.-]((19|[2-9]\d)\d{2})\s(0[0-9]|1[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9]))|((0[1-9]|[12]\d|30)[\/\.-](0[13456789]|1[012])[\/\.-]((19|[2-9]\d)\d{2})\s(0[0-9]|1[0-9]|2[0-3]):([0-5][0-9])))$/g;
// if (datepattern.test(firdatetime)== true) {
//     $("#fir_gddatetime").val(firdatetime);
// }
// else{
...
function datevalidationcheck(datevalue)
{
var datepattern = /((0[1-9])|(1[0-9])|(2[0-9])|(3[0-1]))-((0[0-9])|(1[0-2]))-[0-9]{4} (0\d|1\d|2[0-3]):[0-5]\d/;
if (datepattern.test(datevalue)== true)
{
  return true;
}
else
{
  alert('Wrong Date Format-1')
  return false;
}
};
...
if (datepattern.test(gddatetime)== true) {
    ...
    if (firdate < firgddate){

    document.getElementById('gd_date_error').innerHTML = 'GD date is greater than FIR date';
...
var checkvicjson = Object.keys(victim_list).length
if (checkvicjson == 0) {
document.getElementById("victimtypes2").checked = true;
...
function datecheckvalue(datevalue){
    firdate = $("#fir_date").val();
    firoccinforeceived = datevalue;

var occdatecheck = firoccinforeceived
// alert(firoccinforeceived);
var datepattern = /((0[1-9])|(1[0-9])|(2[0-9])|(3[0-1]))-((0[0-9])|(1[0-2]))-[0-9]{4} (0\d|1\d|2[0-3]):[0-5]\d/;
...
if (datepattern.test(occdatecheck)== true) {
    ...
    var firoccinforeceivedval = datevalue.split(" ")
    var firoccinforeceivedvaldate = firoccinforeceivedval[0].split("-")
    var firoccinforeceivedvaldatetime = firoccinforeceivedvaldate[2] + "-" + firoccinforeceivedvaldate[1] + "-" + firoccinforeceivedvaldate[0] + "T" + firoccinforeceivedval[1]
    console.log(firoccinforeceivedvaldatetime);

    if (arrfirdatetime < firoccinforeceivedvaldatetime){
    alert('FIR Date is Less Than Entered Date'+ firoccinforeceived)  
    return false
    }
    else{
        return true 
...
function load_resource(url, data, elm_id, val_pos, text_pos, value){
    ...
    $.each(data, function(key, resource) {
     ...
     if(district_edit != "" && elm_id == "fir_comp_district"){
        $("#fir_comp_district").val(district_edit);
        district_edit = "";
    }
    if(ps_edit != "" && elm_id == "fir_comp_police_station"){
        $("#fir_comp_police_station").val(ps_edit);
        ps_edit = "";
    }
    if(district_same_edit != "" && elm_id == "fir_compl_sameasdistrict"){
        $("#fir_compl_sameasdistrict").val(district_same_edit);
        district_same_edit = "";
    }
    if(ps_same_edit != "" && elm_id == "fir_compl_sameaspolicestation"){
        $("#fir_compl_sameaspolicestation").val(ps_same_edit);
        document.getElementById("fir_compl_sameaspolicestation").value = ps_same_edit;
        // ps_same_edit = "";
    }
...
function load_resource_ps(url, data, elm_id, val_pos, text_pos, value){
    if(elm_id != 'id_state'){show();};
    reset_select(elm_id, value);
    $.ajax({ url: url, data: data, success: function(data){
    $.each(data, function(key, resource) {
...
function load_accused(row){
    ...
    if(row.is_acc_police=="Y"){
        ...
        // $("#fir_accusedpolice_name").val('');
        // reset_text(["fir_accusedname"]);
...
function load_occ(row){
    ...
    $("#occlongitude").val(row.fir_occ_long);
    $("#occlatitude").val(row.fir_occ_lat);
...
// var checkvalue = $("fir_comp_sameasper2").val();
var checkvalue = document.getElementById('fir_comp_sameasper2').value;

if (checkvalue == 'No')
{
// alert(checkvalue);
document.getElementById("fir_comp_samesasaddress").disabled = false;  
document.getElementById("fir_compl_sameascountry").disabled = false; 
document.getElementById("fir_compl_sameasstate").disabled = false; 
document.getElementById("fir_compl_sameasdistrict").disabled = false; 
document.getElementById("fir_compl_sameaspolicestation").disabled = false; 
$("#fir_compl_sameasstate").blur(function() {

        data = {"state": $("#fir_compl_sameasstate").val(), "district": district_edit}
        load_resource(district_url, {"state": $("#fir_compl_sameasstate").val()}, "fir_compl_sameasdistrict", 0, 1, "Select District");
        // alert(police_station_url);
...
$("#fir_comp_state").blur(function() {
    if(district_edit != "" && ps_edit != "" ){
        data = {"state": $("#fir_comp_state").val(), "district": district_edit}
        load_resource(district_url, {"state": $("#fir_comp_state").val()}, "fir_comp_district", 0, 1, "Select District");
...
//     if(district_same_edit != "" && ps_same_edit != "" ){
//         data = {"state": $("#fir_compl_sameasstate").val(), "district": district_edit}
//         load_resource(district_url, {"state": $("#fir_compl_sameasstate").val()}, "fir_compl_sameasdistrict", 0, 1, "Select District");
//         load_resource(police_station_url, data, "fir_compl_sameaspolicestation", 0, 1, "Select Police Station");
//     }else{
...
$("#fir_compl_sameasstate").blur(function() {
    ...
    if(district_same_edit != "" && ps_same_edit != "" ){
        data = {"state": $("#fir_compl_sameasstate").val(), "district": district_same_edit}
        load_resource(district_url, {"state": $("#fir_compl_sameasstate").val()}, "fir_compl_sameasdistrict", 0, 1, "Select District");
        // alert(data);
        load_resource_ps(police_station_url, data, "fir_compl_sameaspolicestation", 0, 1, "Select Police Station");
    }else{
        data = {"state": $("#fir_compl_sameasstate").val()}
        load_resource(district_url, data, "fir_compl_sameasdistrict", 0, 1, "Select District");
    }
...
$('#act_addRow').click(function(){
    if(($("#fir_acts").find(":selected").text() == 'Acts') || ($("#fir_section").find(":selected").text() == 'Sections')){
        return 0;
    }
    ...
    console.log(fir_act_cd);
    console.log(fir_sec_cd);
    console.log(fir_list);
    
    if(found){return 0;}
    fir_list[fir_list.length] = {'act_cd': fir_act_cd, 'act_text': fir_act_text, 'sec_cd': fir_sec_cd, 'sec_text': fir_sec_text, 'from_db': "No",'soft_delete': "No", "act_srno": ""};
...
function delete_row(data_table_id, row_no){
    ...
    if(fir_list[row_no]['from_db'] == 'Yes'){
        fir_list[row_no]['soft_delete'] = "Yes";
    }
    else{
    ...
    else
{
    ...
    'is_organized': is_organized, 'occ_directionfromps_val': fir_occ_directionfromps_val, 'soft_delete': soft_delete, 'from_db': from_db, 'fir_occ_srno': fir_occ_srno};
...
function delete_occ_row(data_table_id, row_no){
    row_no -= 1;
    $("#"+data_table_id+" tr[id="+row_no+"]").remove();
    if(occ_list[row_no]['from_db'] == 'Yes'){
        occ_list[row_no]['soft_delete'] = "Yes";
    }else{
    ...
    else
{
    ...
    // alert(document.getElementById("istimeofoccur2").checked);
    // alert($("#fir_occ_info_received_datetime").prop('disabled'));
    // alert($("#fir_occ_datefrom").prop('disabled'));
    // alert(datecheckvalue($("#fir_occ_dateto").val()));
    // alert(document.getElementById("fir_occ_info_received_datetime").value);
    // if (document.getElementById("fir_occ_info_received_datetime").value == 'Unknown' && datecheckvalue($("#fir_occ_dateto").val()) == true)

    if (document.getElementById("istimeofoccur2").checked == true)
    {
    ...
    soft_delete = "No";
    fir_occ_srno = "";
    if(occ_list[row_id]['from_db'] == "Yes"){
        from_db = "Yes";
        fir_occ_srno = occ_list[row_id]['fir_occ_srno']
    }
    else{
        from_db = "No";
    }
    ...
    else if (document.getElementById("fir_occ_istimeofoccur").checked == true && datecheckvalue($("#fir_occ_info_received_datetime").val()) == true && datecheckvalue($("#fir_occ_datefrom").val()) == true && datecheckvalue($("#fir_occ_dateto").val()) == true)
    {
    fir_occ_datefrom = "<td>"+$("#fir_occ_datefrom").val()+"</td>";
    fir_occ_dateto = "<td>"+$("#fir_occ_dateto").val()+"</td>";
    fir_occ_directionfromps = "<td>"+$("#fir_occ_directionfromps").find(":selected").text()+"</td>";
...
function create_victim_list(row_id, from='insert'){
    ...
    vic_type = $('input[name="fir_comp_victimtype"]:checked').val();
    ...
    if(from == 'update'){
        if(victim_list[row_id]['from_db'] == "Yes"){
            from_db = "Yes";
            fir_victim_srno = victim_list[row_id]['fir_victim_srno']
        }else{
            from_db = "No";
        }
    }
    ...
    'from_db': from_db, 'soft_delete': soft_delete, 'fir_victim_srno': fir_victim_srno, 
...
$('#victim_addRow').click(function(){
    ...
    document.getElementById("fir_victimreligion_error").innerHTML = "";
    document.getElementById("fir_victimcastcat_error").innerHTML = "";
    document.getElementById("fir_victiminjury_error").innerHTML = "";
...
function create_accused_list(row_id, from='insert'){
    ...
    acc_accusedpolice_name = "<td>"+$("#fir_accusedpolice_name").val()+"</td>";
    acc_police_rank = $("#fir_accusedpolice_rank").find(":selected").text();
    ...
    if(from == 'update'){
        if(accused_list[row_id]['from_db'] == "Yes"){
            from_db = "Yes";
            acc_srno = accused_list[row_id]['acc_srno'];
        }else{
            from_db = "No";
        }
    }
    accused_list[row_id] = {'is_acc_police': is_acc_police, 'acc_accusedpolice_name': acc_accusedpolice_name, 'acc_police_rank': acc_police_rank, 'acc_police_rank_val': acc_police_rank_val, 'accusedname': fir_accusedname, 'accaddress': fir_accaddress, 'acc_type': acc_type,
    ...
    'soft_delete': 'No', 'from_db': from_db, 'acc_srno': acc_srno}
...
function update_accused(row_id){
  ...
  // alert($('#fir_accusedpolice_rank').is(':disabled'));
    ...
    else if($("#fir_accusedpolice_name").val() == "" && $('fir_accusedpolice_name').prop('disabled') == false)
    {
    document.getElementById("fir_accusedpolice_name_error").innerHTML = "Enter Police Personal Name";
    document.getElementById("fir_accgender_error").innerHTML = "";
    document.getElementById("fir_accagetype_error").innerHTML = "";
    document.getElementById("fir_accyob_error").innerHTML = "";
    document.getElementById("fir_accaddress_error").innerHTML = "";
    ...
    document.getElementById("fir_accusedpolice_name").focus();

        return 0;
    }

    else if($("#fir_accusedpolice_rank").find(":selected").val() == "" && $('#fir_accusedpolice_rank').is(':disabled') == false)
{

    document.getElementById("fir_accusedpolice_rank_error").innerHTML = "Select Rank";
    document.getElementById("fir_accgender_error").innerHTML = "";
    document.getElementById("fir_accagetype_error").innerHTML = "";
    document.getElementById("fir_accyob_error").innerHTML = "";
    document.getElementById("fir_accaddress_error").innerHTML = "";
...
function create_property_list(row_no, from="insert"){
    if(($("#fir_property_category").find(":selected").text() == 'Select Category') || ($("#fir_property_type").find(":selected").text() == 'Select Type')){
        return 0;
    }
    ...
    prop_reg_num = "";
    if(from == 'update'){
        if(property_list[row_no]['from_db'] == "Yes"){
            from_db = "Yes";
            prop_reg_num = property_list[row_no]['prop_reg_num']
        }else{
            from_db = "No";
        }
    }
    ...
    'category_val': fir_property_category_val, 'type_val': fir_property_type_val, 'nature_val': fir_property_nature_val, 'from_db': from_db, 'soft_delete': soft_delete, 'prop_reg_num': prop_reg_num}
    html = build_rows_prop(property_list);
    $("#prpoerty_datatable").html(html);
    reset_property();
}
...
function delete_prop_row(data_table_id, row_no){
    ...
    $("#"+data_table_id+" tr[id="+row_no+"]").remove();
    if(property_list[row_no]['from_db'] == 'Yes'){
        property_list[row_no]['soft_delete'] = "Yes";
    }else{
        property_list.splice(row_no, 1);
    }
    $("#prpoerty_datatable").html(build_rows_prop(property_list));
}
...
function copy_compliant_to_victim(){
    ...
    vic_type = "Known";
    vic_type_category = $('input[name="fir_comp_victimtype"]:checked').val();
    vic_medical = $('input[name="fir_victimmedical"]:checked').val();
...
function enable_disable_accused(status){
    ...
    // alert(status)
    // alert($('input[name=fir_accusedpolice]:checked').val());
