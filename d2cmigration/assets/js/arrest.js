...
function build_existing_acc(existing_accused){
    ...
    existing_accused.forEach(function(row_e, index){
        ...
        no_e = '<td>'+index_no+'</td>'
        html = '<tr id="'+index_no+'">'+no_e+row_e.accusedname+row_e.acc_relname+row_e.acc_ps+add_button_e+edit_button_e+delete_button_e+'</tr>';
        rows_e.push(html)
    });
    return rows_e;
}
...
function build_rows_acc(accused_list){
    rows = []
    header = "<thead class='thead-dark'><th>S.No.</th><th>Name</th><th>Relative Name</th> <th>PS</th><th>Arrest Or Surrender Details</th><th>Arrest Personal Details</th> <th>Delete</th></thead>"
    accused_list.forEach(function(row, index){
        index += 1;
        add_button = "<td><a class='btn btn-icon btn-success mr-1 mb-1' title='AddStatement' data-toggle='modal' data-target='#inlineForm' onclick='cur_acc_index ="+ index+";reset_arrest_details();load_arrest_details();'><i class='step-icon feather icon-plus-square'></i>&nbsp;Add</a></td>";
        // acc_add_button = "<td><a class='btn btn-icon btn-success mr-1 mb-1' title='AccInfo' data-toggle='modal' data-target='#editaccusedmodal' onclick='cur_acc_index ="+ index+";reset_acc_info();load_acc_info();'><i class='step-icon feather icon-plus-square'></i>&nbsp;Add</a></td>";
        ...
        no = '<td>'+index+'</td>'
        html = '<tr id="'+index+'">'+no+row.accusedname+row.acc_relname+row.acc_ps+add_button+edit_button+delete_button+'</tr>';
        rows.push(html)
    });
    existing_accused_list_row = build_existing_acc(existing_accused);
    return header+"<tbody>"+rows+existing_accused_list_row+"</tbody>";
//    return header+"<tbody>"+rows+"</tbody>";
}
...
function create_accused_list(row_id, from='insert'){
    ...
    if ($("#acc_reltype").find(":selected").text() == '')
    {
    acc_rel_type_val = '0'        
    }
    else{
    ...
    if(from == 'insert'){
        ...
        isfingerprinttaken =  '0';
        accheight =  '';
        isdangerous = '0';
        isclothmalefemale = '0';
        isgenerallyarmed = '0';
        isbailjumpprevious = '0';
        isbailjumppreviousdetail = '0';

        isgenerallyarmed =  '0';
        isknowncriminal = '0';
        isknowncriminaldetails = '';
        isrecidivist = '0';
    ...
    // 'isarrestorsurrender': '', 'arrestdatetime': '', 'arrestgdnumber': '', 'arrstate_val': '', 'arrdistrict_val': '', 'arrps': '',
    // 'accusedtypeofcourt_val': '', 'accusedmegistrate': '', 'accusedactiontaken_val': '', 'accheight': '', 'accheighttocm': '',
    // 'acctattoodesc': '', "accbuildtype": "0", "acccomplexion": "0", "accearmissing": "0", "acceardeform": "0", "acceardeaf": "0", 
    // "accarmmissing": "0", "accfingermissing": "0", "acclegmissing": "0", "acchunchback": "0", "acctoeextra": "0", "acctoemissing": "0", 
    // "acclimping": "0", "accbowleg": "0", "accknockknee": "0", "accearlobs": "0", "accgoitre": "0", "accteethtype": "0", "acchairtype": "0", 
    // "accwigtype": "0", "acchairstrt": "0", "acchaircolor": "0", "acchaircut": "0", "acchairstyle": "0", "acchairlength": "0", "acchairdye": "0", 
    // "acchaircolor": "0", "acceyetype": "0", "acceyeblind": "0", "acceyecolor": "0", "acceyespec": "0", "acceyesquint": "0", "acceyebrow": "0", 
    // "acceyebrowshade": "0", "acceyeblink": "0", "acchabits": "0", "acchabitsdress": "0", "accdialect": "0", "accplaceburnmark": "0", "accplaceleukoderma": "0", 
    // "accplacemole": "0", "accplacescar": "0", "accplacetattoo": "0", "accfacetype": "0", "acccheektype": "0", "accforheadtype": "0", "accchintype": "0", 
    // "accmustachetype": "0", "accbearedtype": "0", "accnosetype": "0", "acclipstype": "0", "accpoxpitted": "0", "from_db": "No", "soft_delete": "No","accused_srno":'', 'module': '' }

    accused_list[row_id] = {'accusedname': fir_accusedname, 'accaddress': fir_accaddress, 'acc_type': acc_type, 
    'acc_rel_type': acc_rel_type, 'acc_rel_type_val': acc_rel_type_val, 'acc_relname': acc_relname,  'acc_religion_val': acc_religion_val, 'acc_caste': acc_caste, 'acc_caste_val': acc_caste_val, 
    'acc_rel_type': acc_rel_type, 'acc_rel_type_val': acc_rel_type_val, 'acc_relname': acc_relname, 
    ...
    'acc_district_same': acc_district_same, 'acc_district_same_val': acc_district_same_val, 'acc_ps_same': acc_ps_same,
    'acc_ps_same_val': acc_ps_same_val, 'acc_nationality': acc_nationality, 'acc_nationality_val': acc_nationality_val, 'acc_sameasper': acc_sameasper,
    'isarrestorsurrender': isarrestorsurrender, 'isfingerprinttaken': isfingerprinttaken, 'arrestdatetime': arrestdatetime, 'arrestpsinfodatetime': arrestpsinfodatetime, 'arrestgdnumber': arrestgdnumber, 'arrstate_val': arrstate_val, 'arrdistrict_val': arrdistrict_val, 'arrps': '', 'arrps_val': arrps_val,
    ...
    'accusedtypeofcourt_val': accusedtypeofcourt_val, 'accusedmegistrate': accusedmegistrate, 'accusedactiontaken_val': accusedactiontaken_val, 'accusedactiontakenarr_val': accusedactiontakenarr_val, 'accheight': accheight, 'accheighttocm': accheighttocm,
    ...
    "acceyebrowshade": acceyebrowshade, "acceyeblink": acceyeblink, "acchabits": acchabits, "acchabitsdress": acchabitsdress, "accdialect": accdialect, "accplaceburnmark": accplaceburnmark, "accplaceleukoderma": accplaceleukoderma, 
    ...
    "accmustachetype": accmustachetype, "accbearedtype": accbearedtype, "accnosetype": accnosetype, "acclipstype": acclipstype, "accpoxpitted": accpoxpitted, "from_db": from_db, "soft_delete": soft_delete,"accused_srno":accused_srno, 'module': '' }
...
function load_accused(row){
    ...
    $("#acccastecat").val(row.acc_caste_val).trigger("change");
    ...
    else if($("#accusedmegistrate").val() == "" && $("#accusedmegistrate").prop('disabled') == false)
{
    ...
    else if($("#accusedactiontaken").find(":selected").val() == "" && $("#accusedactiontaken").prop('disabled') == false)
{
    ...
    else if($("#accusedactiontakenarr").find(":selected").val() == "" && $("#accusedactiontakenarr").prop('disabled') == false)
{
...
function create_witness_list(row_id, from='insert'){
    ...
    witness_list[row_id] = {'witness_name': witness_name, 'witness_address': witness_address, 'witness_evidence_val':witness_evidence_val, 'witness_country_val': witness_country_val, 'witness_state_val': witness_state_val,
    'witness_rel_type': witness_rel_type, 'witness_rel_type_val': witness_rel_type_val, 'witness_relativename': witness_relativename, 'witness_nationality_val': witness_nationality_val,  
    ...
    'witness_agemonth': witness_agemonth, 'witness_agefrom': witness_agefrom, 'witness_ageto': witness_ageto, 'witness_agetype_val': witness_agetype_val, 'from_db': from_db, 'soft_delete': soft_delete};
...
$('#add_witness').click(function(){
    ...
    document.getElementById("fir_witnessaname_error").innerHTML = "";        
    }
