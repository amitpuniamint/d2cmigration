        ...
        if(form.attr("id") == "fir_form"){
            ...
            if(newIndex == 2){
                ...
                if(victim_list.length > 0){
                    ...
                    if($('input[name="fir_comp_victimtype"]:checked').val() == "Unknown"){
                        return true
                    }
                    return false;
                }
             ...
             // Custom check and validate form
             if(newIndex == 1){
    var rowCount = $('#arrest_act_sec tbody tr').length;
    if(rowCount > 0){
    } else {
        // alert(rowCount);
        $('#act_section_modal_message').modal('show');
        return false;
        ...
        // return false;
             ...
             if(newIndex == 2){
                ...
                var rowCount = $('#accused_table tbody tr').length;
                    ...
                    $('#accused_add_modal_message').modal('show');
                    return false;
                    }
    ...
    onFinished: function (event, currentIndex) {
        if(form.attr("id") == "fir_form"){
           ...
           form.submit(function(e) {
                    ...
                    var firdate = document.getElementById("fir_date").value;
                    if (datevalidationcheck(firdate)==true)
                    {
                        $("a[href$='finish']").hide(); 
...
if(res_enabled === false)
{
    if (nonres_enabled === false)
    {
    alert('Pleae Enter ChargeSheet/Non ChargeSheet Details Before Submit!');
    // $('#chs_datacheck_message').modal('show');
    return false;        
    }
    else if (nonres_enabled === true)
    {
    if (accused_list[i]['accusedaddverified'] == '')
    {
        alert('Accused ChargeSheet Information Missing!')
        return false;
    }
...
else if(res_enabled === true)
{
    if (nonres_enabled === true)
    {
    alert('Use Clear Button To Fill ChargeSheet/Non ChargeSheet Details Before Submit!');
    return false;
    }
    else if (nonres_enabled === false)
    {
    if (accused_list[i]['accusedaddverified'] == '')
    {
        // $('#chs_datacheck_message').modal('show');
        // alert('Accused NonChargeSheet Information Missing!')
        // return false;
    }
