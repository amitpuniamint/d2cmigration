...
function load_resource(url, data, elm_id, val_pos, text_pos, value){
    ...
    if(person_district_edit != "" && elm_id == "person_district"){
        $("#person_district").val(person_district_edit);
        person_district_edit = "";
    }
    if(person_ps_edit != "" && elm_id == "person_ps"){
        $("#person_ps").val(person_ps_edit);
        person_district_edit = "";
    }
  }});
}
...
function load_person_district_resource(url, data, elm_id, val_pos, text_pos, value){
    reset_select(elm_id, value);
    $.ajax({ url: url, data: data, success: function(data){
    $.each(data, function(key, resource) {
     $('#'+elm_id).append($("<option></option>").attr("value",resource[val_pos]).text(resource[text_pos]));
...
$("#sei_state").blur(function() {
    ...
    load_sei_district_resource(district_url, data, "sei_district", 0, 1, "Select District");

    if(district_edit != "" && ps_edit != "" ){
        data = {"state": $("#sei_state").val(), "district": district_edit}
        load_sei_district_resource(police_station_url, data, "sei_ps", 0, 1, "Select Police Station");
    }else{
        load_sei_district_resource(district_url, data, "sei_district", 0, 1, "Select District");
    }
});
...
$("#person_state").blur(function() {
    ...
    load_person_district_resource(district_url, data, "person_district", 0, 1, "Select District");

    if(person_district_edit != "" && person_ps_edit != "" ){
        data = {"state": $("#person_state").val(), "district": person_district_edit}
        load_person_district_resource(police_station_url, data, "person_ps", 0, 1, "Select Police Station");
    }else{
        load_person_district_resource(district_url, data, "person_district", 0, 1, "Select District");
    }
});
...
function create_witness_list(row_id, from='insert'){
    ...
    'witness_nationality_val': witness_nationality_val, 'witness_evidence_val': witness_evidence_val, 'witness_srno': witness_srno, 'from_db': from_db, 'soft_delete': soft_delete}
...
function load_witness(row){
    ...
    $("#witnessoccupation").val(row.witness_occupation).trigger("change");
...
function submitDataTableValues(){
    ...
    $("<input />").attr("type", "hidden")
                  ...
                  .appendTo("#seizure_add_form");
    $("<input />").attr("type", "hidden").attr("name", "seizureno").attr("value",$("#seizureno").val()).appendTo("#seizure_add_form")
 }
