...
var firdatetime = document.getElementById("cyberaquisitiondate").value;
...
$(document).ready(function() {
    ...
    $("#date").attr("max",set_time_picker_min_val_today());
    $("#autoregistrationdate").attr("max",set_time_picker_min_val_today());
    // $("#fir_gddatetime").attr("max",document.getElementById('fir_date'));

    $('input[name="cyberdigitalmedia"]').prop("disabled", true);
    $('input[name="ifdriveimaged"]').prop("disabled", true);
    $('input[name="cybernetworkaquisition"]').prop("disabled", true);

$('crime_add_form').submit(function(){
...
var act_sec_major_head = [];
$.each(act_sec_list, function(key,value) {
    if(value.soft_delete == 'No')
    {
var res = value.sec_cd.replace("<th>", "");
var res = res.replace("</th>", "");
    act_sec_major_head.push({
        sec_cd: res
    });
}
    console.log(JSON.stringify(act_sec_major_head));
}); 
...
// Called when District & PS to be loaded during edit functionality
function load_majorhead(section_code){
    data = {"section_code":section_code};
    reset_select(major_head_code, "Select MajorHead");
    $.ajax({ url: majorhead_url,
             data: data,
             success: function(data){
                     $.each(data, function(key, resource) {
                         if(resource[0] == majorhead_val){
                            $('#'+major_head_code).append($("<option></option>").attr("value",resource[0]).attr('selected', 'selected').text(resource[1]));
                         }else{
                            $('#'+major_head_code).append($("<option></option>").attr("value",resource[0]).text(resource[1]));
                         }
                      });
...
function load_resource(url, data, elm_id, val_pos, text_pos, value){
    ...
    if(district_edit != "" && elm_id == "crime_district"){
        $("#crime_district").val(district_edit);
        district_edit = "";
    }
    if(ps_edit != "" && elm_id == "crime_ps"){
        $("#crime_ps").val(ps_edit);
        ps_edit = "";
    }

    if(same_district_edit != "" && elm_id == "occ_sameasdistrict"){
        $("#occ_sameasdistrict").val(same_district_edit);
        same_district_edit = "";
    }

    if(same_ps_edit != "" && elm_id == "occ_sameasps"){
        $("#occ_sameasps").val(same_ps_edit);
        same_ps_edit = "";
    }


    if(district_pl_edit != "" && elm_id == "crime_pldistrict"){
        $("#crime_pldistrict").val(district_pl_edit);
        district_pl_edit = "";
    }
    if(ps_pl_edit != "" && elm_id == "crime_plps"){
        $("#crime_plps").val(ps_pl_edit);
        ps_pl_edit = "";
    }
  }});
}

function load_resource_occ_sameadd(url, data, elm_id, val_pos, text_pos, value){
    reset_select(elm_id, value);
    $.ajax({ url: url, data: data, success: function(data){
    $.each(data, function(key, resource) {
        $('#'+elm_id).append($("<option></option>").attr("value",resource[val_pos]).text(resource[text_pos]));
...
$("#crime_state").blur(function() {
    if(district_edit != "" && ps_edit != "" ){
        data = {"state": $("#crime_state").val(), "district": district_edit}
...
$("#occ_sameasstate").blur(function() {
    if(same_district_edit != "" && same_ps_edit != "" ){
        data = {"state": $("#occ_sameasstate").val(), "district": same_district_edit}
        load_resource_occ_sameadd(district_url, {"state": $("#occ_sameasstate").val()}, "occ_sameasdistrict", 0, 1, "Select District");
        load_resource_occ_sameadd(police_station_url, data, "occ_sameasps", 0, 1, "Select Police Station");
    }else{
        load_resource_occ_sameadd(district_url, {"state": $("#occ_sameasstate").val()}, "occ_sameasdistrict", 0, 1, "Select District");
    }
});
...
$("#crime_plstate").blur(function() {
    if(district_pl_edit != "" && ps_pl_edit != "" ){
        data = {"state": $("#crime_plstate").val(), "district": district_pl_edit}
        load_resource(district_url, {"state": $("#crime_plstate").val()}, "crime_pldistrict", 0, 1, "Select District");
...
$("#occ_sameasdistrict").blur(function() {
  data = {"state": $("#occ_sameasstate").val(), "district": $("#occ_sameasdistrict").val()}
  load_resource(police_station_url, data, "occ_sameasps", 0, 1, "Select Police Station");
});
...
$('#ma_mi_addRow').click(function(){
    ...
    'type_of_crime': type_of_crime, 'soft_delete': '', 'crm_det_srno': ''};
...
function delete_row_major_minor(data_table_id, row_no){
    row_no -= 1;
    $("#"+data_table_id+" tr[id="+row_no+"]").remove();
    if(major_minor_list[row_no]['from_db'] == 'Yes'){
        major_minor_list[row_no]['soft_delete'] = "Yes";
    }
    else{
...
function create_accused_list(row_id, from='insert'){
    ...
    'from_db': from_db,'soft_delete': 'No', "acc_srno": acc_srno}
...
function create_victim_list(row_id, from='insert'){
    ...
    'vic_type_category': vic_type_category, 'vic_roadacc': vic_roadacc, 'vic_medical':vic_medical, 
    'vic_occupation': vic_occupation, 'vic_occupation_val': vic_occupation_val, 
    'vic_gender': vic_gender, 'vic_gender_val': vic_gender_val, 'vic_age_proof_val': vic_age_proof_val, 'vic_age_details': vic_age_details, 
...
$('#victim_addRow').click(function(){
    ...
    document.getElementById("victim_occupation_error").innerHTML = "";
    ...
    document.getElementById("victim_religion_error").innerHTML = "";
    document.getElementById("victim_castecat_error").innerHTML = "";
    document.getElementById("victim_injury_error").innerHTML = "";
...
function create_witness_list(row_id, from='insert'){
    ...
    'witness_sameaddress': witness_sameaddress, 'witness_sameasper': witness_sameasper, 'witness_srno': witness_srno, 'from_db': from_db, 'soft_delete': soft_delete}
...
//    if(found && from != 'update'){return 0;}
    ...
    soft_delete = "No";
    if(from == 'update'){
        if(property_list[row_id]['from_db'] == "Yes"){
            from_db = "Yes";
            prop_reg_num = property_list[row_id]['prop_reg_num']
            soft_delete = property_list[row_id]['soft_delete']
        }else{
            from_db = "No";
        }
    }
    property['from_db'] = from_db;
    property['prop_reg_num'] = prop_reg_num;
    property['soft_delete'] = soft_delete;
...
function reset_property(){
    ...
    "jewelleryweightingms", "otherslostvalue", "othersdescription", "wildotherdesc", "cyberaquiredby", "cyberprotdevicetype", "cyberaquisitiondate", "autochasisno", "autoregnumber", "autoenginenumber"]);
    ...
    "cyberaquisitiondetail2", "cybernetworkaquisition2", "cyberismobiledevice2", "ischasisnoknown2", "isenginenoknown2", "isregnoknown2", "isregnoknown2", "ischasisnoknown2", "isenginenoknown2"]);
...
function load_property(row){
    ...
    switch(category_case){
        ...
        case "DRUGS/NARCOTIC DRUGS":
            ...
            else if(row.drugaccusedmember == "Y")
            {
                $("#drugaccusedmember1").click();
            }
            else{
            ...
            if(row.drugjointinterrogation == "Y")
            {
                $("#drugjointinterrogation1").click();
            }
            else
        ...
        case "EXPLOSIVES":
            ...
            else if(row.explosivesenttofsl == "Y"){
                $("#explosivesenttofsl1").click();
            }
            else{
            ...
            else if(row.explosivedestroyed == "Y"){
                $("#explosivedestroyed1").click();
            }
            else{
...
function enable_disable_victim(status){
    ...
    $("#victim_occupation").prop("disabled", status);
